...
interface UseKeepAliveTabsProps {
  menu: any[];
...
export function useKeepAliveTabs(props: UseKeepAliveTabsProps) {
  ...
  const breadcrumbNameMap = useMemo(
    () => getBreadcrumbNameMap([{ ...menu }], []),
    [menu],
  );

  const matchRoute = useMatchRoute({breadcrumbNameMap, children});
